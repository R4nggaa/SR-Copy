...
const repository = settings.useCustomRepo ? settings.customRepo : 'Dimbreath/turnbasedgamedata'
const apiBase = 'https://gitlab.com/api/v4/projects/' + encodeURIComponent(repository)
const jsonBase = 'https://gitlab.com/' + repository + '/-/raw/'

export function getRepositoryFileUrl(commitId:string, path:string)
{
    return `https://gitlab.com/${repository}/-/blob/${commitId}/${path}`
}
